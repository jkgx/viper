...
func WatchConfig(l *logrus.Logger, o *WatchOptions) {
	if l == nil {
		l = logrus.New("", "")
	}
...
func InitializeConfig(applicationName string, homeOverride string, l *logrus.Logger) *logrus.Logger {
	if cfgFile != "" {
		...
		if err != nil {
			logrus.New(applicationName, "").WithField("error", err.Error()).Fatal("Unable to locate home directory")
		}
